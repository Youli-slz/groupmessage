  ...
  <div style="width: 100%">
      ...
      <el-dialog title="添加章节" v-model="dialogVisible" size="small">
        ...
        <el-form-item label="选择">
            <el-select v-model="flag1" placeholder="选择">
              <el-option label="故事" value="-1"></el-option>
              <el-option label="读后感" value="-2"></el-option>
            </el-select>
          </el-form-item>
      ...
      <el-dialog title="修改章节" v-model="dialogVisibleup" size="small">
        <el-form label-width="80px">
          <el-form-item label="选择">
            <el-select v-model="flagup" placeholder="选择">
              <el-option label="故事" value="-1"></el-option>
              <el-option label="读后感" value="-2"></el-option>
            </el-select>
          </el-form-item>
...
export default {
  ...
  data() {
    ...
    methods: {
      delet: function (val) {
        var self = this
        this.axios.post(this.http + '/index.php?r=readweb/deldushu', {articleId: val, flag: this.flag})
              .then((response) => {
      ...
      searchbook() {
        var self = this
        this.axios.post(this.http + '/index.php?r=readweb/getdushu', {flag: this.flag})
              .then((response) => {
                var data = response.data
                if(data.state == 1000){
      ...
      addbook() {
        ...
        this.axios.post(this.http + '/index.php?r=readweb/adddushu', {flag: this.flag1, articleName: this.title, articlePic: this.img, articleDis: this.dec, articleUrl: this.link})
              .then((response) => {
                var data = response.data
                if(data.state == 1000){
                  self.$message("添加成功") 
      ...
      upbook() {
        ...
        this.axios.post(this.http + '/index.php?r=readweb/updushu', {flag: this.flagup,articleId: this.articleId, articleName: this.titleup, articlePic: this.imgup, articleDis: this.decup, articleUrl: this.linkup})
              .then((response) => {
                var data = response.data
                if(data.state == 1000){
                  self.$message("修改成功") 
